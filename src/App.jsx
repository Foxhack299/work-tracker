  ...
  return (
    <div style={{ fontFamily:"'Courier New',monospace", background:"#0a0a0a", minHeight:"100vh", color:"#e8e0d0", maxWidth:430, margin:"0 auto", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ padding:"32px 24px 16px", borderBottom:"1px solid #222", background:"#0a0a0a", position:"sticky", top:0, zIndex:10 }}>
        ...
        <div style={{ fontSize:30, fontWeight:"bold", letterSpacing:-1, color:"#f0e8d8" }}>Temps de Travail</div>
      </div>
      ...
      <div style={{ flex:1, overflowY:"auto", paddingBottom:100 }}>
            ...
            {(() => {
              ...
              return (
                <div style={{ background:"#111", border:"1px solid #2a2a2a", borderRadius:4, padding:20, marginBottom:16 }}>
                  <div style={{ fontSize:12, letterSpacing:3, color:"#666", marginBottom:16 }}>RÉCAPITULATIF DU JOUR</div>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
                    <div style={{ background:"#0a0a0a", borderRadius:4, padding:"14px 16px" }}>
                      ...
                      <div style={{ fontSize:30, fontWeight:"bold", color: lastSession ? "#f0e8d8" : "#333", fontVariantNumeric:"tabular-nums" }}>
                  ...
                  <div style={{ background:"#0a0a0a", borderRadius:4, padding:"12px 16px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    ...
                    <span style={{ fontSize:24, fontWeight:"bold", color: todaySeconds > 0 ? "#4ade80" : "#333" }}>
        ...
        {tab==="calendar" && (
          <div style={{ padding:24 }}>
            <div style={{ fontSize:12, letterSpacing:3, color:"#666", marginBottom:16 }}>SAISIE MANUELLE</div>

            {/* Taux horaire */}
            <div style={{ background:"#111", border:"1px solid #2a2a2a", borderRadius:4, padding:16, marginBottom:20 }}>
              <div style={{ fontSize:12, letterSpacing:3, color:"#666", marginBottom:12 }}>TAUX HORAIRE</div>
              {editingRate ? (
        ...
        {tab==="stats" && (
          <div style={{ padding:24 }}>
            <div style={{ fontSize:12, letterSpacing:3, color:"#666", marginBottom:16 }}>STATISTIQUES</div>
            ...
            {[
              ...
              <div key={i} style={{ background:"#111", border:"1px solid #2a2a2a", borderRadius:4, padding:20, marginBottom:12 }}>
                <div style={{ fontSize:12, letterSpacing:3, color:"#666", marginBottom:12 }}>{card.label}</div>
                <div style={{ display:"flex", justifyContent:"space-between" }}>
                  <div>
                    <div style={{ fontSize:36, fontWeight:"bold", color:"#f0e8d8" }}>{card.hours}h</div>
                    <div style={{ fontSize:13, color:"#555", marginTop:2 }}>{card.n} session(s)</div>
                  </div>
                  <div style={{ textAlign:"right" }}>
                    <div style={{ fontSize:36, fontWeight:"bold", color:"#4ade80" }}>{card.earned}€</div>
                    <div style={{ fontSize:13, color:"#555", marginTop:2 }}>salaire estimé</div>
                  </div>
                </div>
              </div>
            ))}
            ...
            {(() => {
              ...
              return (
                <div style={{ background:"#111", border:"1px solid #2a2a2a", borderRadius:4, padding:20, marginBottom:12 }}>
                  <div style={{ fontSize:12, letterSpacing:3, color:"#666", marginBottom:12 }}>TOTAL PAR PÉRIODE</div>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                    <div>
                      <div style={{ fontSize:36, fontWeight:"bold", color:"#f0e8d8" }}>{(filteredSeconds/3600).toFixed(1)}h</div>
                      <div style={{ fontSize:13, color:"#555", marginTop:2 }}>{filteredSessions.length} session(s)</div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:36, fontWeight:"bold", color:"#4ade80" }}>{((filteredSeconds/3600)*hourlyRate).toFixed(2)}€</div>
                      <div style={{ fontSize:13, color:"#555", marginTop:2 }}>salaire estimé</div>
                    </div>
                  </div>
                </div>
        ...
        {tab==="history" && (
          <div style={{ padding:24 }}>
            <div style={{ fontSize:12, letterSpacing:3, color:"#666", marginBottom:16 }}>HISTORIQUE</div>
            {days.length===0 ? (
                ...
                {day.sessions.map(s=>(
                  <div key={s.id} style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:4, padding:"12px 16px", marginBottom:6, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <div>
                      ...
                      <div style={{ fontSize:13, color:"#555", marginTop:2 }}>{formatHM(s.duration)}{s.breakMin>0&&` · pause ${s.breakMin}min`}</div>
                    </div>
          ...
          <div style={{ padding:24 }}>
            ...
            <div style={{ background:"#111", border:"1px solid #2a2a2a", borderRadius:4, padding:20, marginBottom:16 }}>
              ...
              <div style={{ fontSize:13, color:"#555", marginBottom:16, lineHeight:1.6 }}>
                Télécharge un fichier contenant toutes tes sessions et ton taux horaire. À faire avant de changer de téléphone ou de réinstaller l'app.
              </div>
      ...
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:430, background:"#0d0d0d", borderTop:"1px solid #1e1e1e", display:"flex", padding:"12px 0 24px" }}>
        {[
          ...
          <button key={t.id} onClick={()=>setTab(t.id)} style={{ flex:1, background:"transparent", border:"none", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:4, padding:"8px 0" }}>
            ...
            <span style={{ fontSize:11, letterSpacing:0.5, color:tab===t.id?"#f0e8d8":"#444", fontFamily:"'Courier New',monospace", transition:"color 0.2s" }}>{t.label.toUpperCase()}</span>
      ...
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        html, body, #root { background:#0a0a0a; margin:0; padding:0; border:none; }
